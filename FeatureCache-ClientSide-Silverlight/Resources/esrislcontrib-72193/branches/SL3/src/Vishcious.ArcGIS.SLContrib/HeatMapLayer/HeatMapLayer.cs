...
namespace Vishcious.ArcGIS.SLContrib
{
    public class HeatMapLayer:DynamicLayer
    {
        ...
        HeatMapPngRenderer _pngRenderer;
        BitmapImage _image;
        ...
        protected override void GetSource( Envelope extent, int width, int height, DynamicLayer.OnImageComplete onComplete )
        {
            ...
            _pngRenderer = new HeatMapPngRenderer( _currentWidth, _currentHeight, MapPoints );

            ParameterizedThreadStart starter = new ParameterizedThreadStart( ( pngRenderer ) =>
            {
                Stream imageStream = InitalizeImage( pngRenderer as HeatMapPngRenderer );
        ...
        private Stream InitalizeImage( HeatMapPngRenderer pngRenderer )
        {
            pngRenderer.Display();
